...
pub trait StreamingAead {
  /**
    Returns a Write for plaintext.  Any data written to the returned
    channel will be encrypted and the resulting ciphertext written to the provided
    {@code ciphertextDestination}
   
    @param ciphertext_destination the channel to which the ciphertext is written.
    @param associated_data data associated with the plaintext. This data is authenticated
        but not encrypted. It must be passed into the decryption.
   */
  fn new_encrypting_channel(&self, ciphertext_destination: Write, associated_data: &[u8]) -> Result<Write>;
   ...
   *     </ul>
   *
   * @throws GeneralSecurityError if the header of the ciphertext is corrupt or if
   *     associatedData is not correct.
   * @throws IOError if an IOError occurred while reading from ciphertextDestination.
   */
  fn new_seekable_decrypting_channel(&self, ciphertext_source: Seek, associated_data: &[u8]) -> Result<Seek>;
  
  fn new_decrypting_channel(&self, ciphertext_source: Read, associated_data: &[u8]) -> Result<Read>;
   ...
   */
  //fn new_decrypting_channel(&self, ...
}
