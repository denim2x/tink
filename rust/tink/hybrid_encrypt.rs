...
/**
  ...
  addition to {@code plaintext} the encryption takes an extra parameter {@code context_info}, which
  usually is public data implicit from the context, but should be bound to the resulting
  ciphertext, i.e. the ciphertext allows for checking the integrity of {@code context_info} (but
  there are no guarantees wrt. the secrecy or authenticity of {@code context_info}).
 
  <p>{@code context_info} can be empty or null, but to ensure the correct decryption of a ciphertext
  the same value must be provided for the decryption operation as was used during encryption (cf.
  {@link HybridEncrypt}).
 
  <p>A concrete instantiation of this interface can implement the binding of {@code context_info} to
  the ciphertext in various ways, for example:
 
  <ul>
    <li>use {@code context_info} as "associated data"-input for the employed AEAD symmetric
        encryption (cf. https://tools.ietf.org/html/rfc5116).
    <li>use {@code context_info} as "CtxInfo"-input for HKDF (if the implementation uses HKDF as key
        derivation function, cf. https://tools.ietf.org/html/rfc5869).
  </ul>
 */
pub trait HybridEncrypt {
  /**
    Encryption operation: encrypts {@code plaintext} binding {@code context_info} to the resulting
    ciphertext.
   
    @return resulting ciphertext
   */
  fn encrypt(&self, plaintext: &[u8], context_info: &[u8]) -> Result<&[u8]>;
}
